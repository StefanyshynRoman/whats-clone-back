...
public class User {
    ...
    public static User fromTokenAttributes(Map<String, Object> attributes, List<String> rolesFromAccessToken) {
        rstefanyshyn.messaging.domain.user.aggregate.UserBuilder userBuilder = rstefanyshyn.messaging.domain.user.aggregate.UserBuilder.user();
        ...
        Set<Authority> authorities = rolesFromAccessToken
                .stream()
                .map(authority -> rstefanyshyn.messaging.domain.user.aggregate.AuthorityBuilder.authority().name(new AuthorityName(authority)).build())
